...
// ss0, ss1, ss2, ss3 ; each one has an associated FIFO
...
// Actually, Sample Sequencer reads the Analog Signal
// FIFO is the conversion result of ADC, also the flag is set when the conversion completes
// 
...
int main()
{
  ...
  SYSCTL->RCGCADC |= 0x01; // Enable clock access to ADC module 0 
	...
	ADC0->ACTSS &=~8; // AlWAYS disbale the Sample Sequencer - during initialization, in this case it is SS3
